...
from companyk_newsbot.ranking import NewsRanker
...
def test_pipeline_runs_article_to_html_with_direct_dedup_and_external_impact() -> None:
    ...
    def summarize(item):
        ...
        return SummaryOutput(fact_summary=f"{item.company} summary.", why_it_matters="Margin pressure." if item.route == "external" else None, insight_one_liner="The next variable is execution.", insight_dimension="strategy", insight_mode="watchpoint", confidence="medium", evidence_article_ids=[article_id(source)])
...
def test_pipeline_summarizes_only_events_that_survive_final_ranking() -> None:
    value = config()
    summarized: list[str] = []
    ...
    def summarize(item):
        summarized.append(item.event_id)
        source = item.direct_match.article if item.direct_match else item.external_match.candidate.article
        from companyk_newsbot.dedup import article_id
        return SummaryOutput(fact_summary="Ranked fact.", why_it_matters="Margin pressure." if item.route == "external" else None, insight_one_liner="Monitor execution.", insight_dimension="strategy", insight_mode="watchpoint", confidence="medium", evidence_article_ids=[article_id(source)])

    pipeline = NewsPipeline(
        route_a_detector=RouteADetector(value),
        route_b_generator=RouteBCandidateGenerator(ExposureRegistry(value)),
        route_b_judge=judge,
        summarize=summarize,
        ranker=NewsRanker(total_max_items=1),
    )
    result = pipeline.run([
        article("Direct Co raises funding", "https://example.com/direct", "Direct Co"),
        article("Google announces Play fee increase", "https://example.com/external", "Google Play fee"),
    ], report_date=date(2026, 8, 12))

    assert len(result.ranked_items) == 1
    assert summarized == [result.ranked_items[0].event_id]


def test_local_pipeline_wires_one_event_resolver_into_both_routes() -> None:
    value = config()
    ...
    class Resolver:
        ...
        def resolve(self, left, right):
            self.calls += 1
            return ResolverResult("SAME_EVENT", "fixture", True)

    resolver = Resolver()

    def judge(candidate):
        return JudgedRouteBCandidate(candidate, JudgeOutput(qualifies=True, company="External Co", exposure_id="play_fee", event_family="platform_infrastructure_dependency", materiality="high", impact_direction="negative", causal_mechanism="Fee affects margin.", rejection_reason="none"), "test", "test")

    def summarize(item):
        source = item.direct_match.article if item.direct_match else item.external_match.candidate.article
        from companyk_newsbot.dedup import article_id
        return SummaryOutput(fact_summary="Fact.", why_it_matters="Impact." if item.route == "external" else None, insight_one_liner="Watch execution.", insight_dimension="strategy", insight_mode="watchpoint", confidence="medium", evidence_article_ids=[article_id(source)])
    ...
    result = NewsPipeline(
        ...
        article("Direct Co market update", "https://example.com/direct-1", "Direct Co"),
        article("Direct Co market outlook", "https://example.com/direct-2", "Direct Co", 9),
        article("Google Play fee market update", "https://example.com/external-1", "Google Play fee"),
        article("Google Play fee market outlook", "https://example.com/external-2", "Google Play fee", 9),
    ], report_date=date(2026, 8, 12))

    assert result.route_a_event_clusters == 1
    assert [item.route for item in result.ranked_items].count("external") == 1
