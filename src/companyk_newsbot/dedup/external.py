...
class ExactIdentityCollapse:
    canonical_article_id: str
    collapsed_link_ids: tuple[str, ...]
    companies: tuple[str, ...]
    exposure_ids: tuple[str, ...]
    source_event_families: tuple[str, ...]
    reason: str = "exact_article_identity"

    def payload(self) -> dict[str, object]:
        return {
            "canonical_article_id": self.canonical_article_id,
            "collapsed_link_ids": list(self.collapsed_link_ids),
            "companies": list(self.companies),
            "exposure_ids": list(self.exposure_ids),
            "source_event_families": list(self.source_event_families),
            "reason": self.reason,
        }
...
class ExternalEventCluster:
    ...
    exact_identity_collapses: tuple[ExactIdentityCollapse, ...]
...
class RouteBEventClusterer:
    ...
    def cluster(self, judged: Iterable[JudgedRouteBCandidate]) -> list[ExternalEventCluster]:
        ...
        groups: list[tuple[list[JudgedRouteBCandidate], list[PairDecision], list[ExactIdentityCollapse]]] = []
        for exact_links, exact_audits, exact_collapses in exact_groups:
            candidate = self._best_article_link(exact_links)
            rejected_audits: list[PairDecision] = []
            for members, audits, collapses in groups:
                ...
                if all(audit.final_decision == "SAME_EVENT" for audit in pair_audits):
                    members.extend(exact_links)
                    audits.extend((*exact_audits, *rejected_audits, *pair_audits))
                    collapses.extend(exact_collapses)
                    break
                rejected_audits.extend(pair_audits)
            else:
                groups.append((list(exact_links), [*exact_audits, *rejected_audits], list(exact_collapses)))
        return [self._event(members, audits, collapses) for members, audits, collapses in groups]
    ...
    def _event(self, links: list[JudgedRouteBCandidate], audits: list[PairDecision], collapses: list[ExactIdentityCollapse]) -> ExternalEventCluster:
        ...
        return ExternalEventCluster(event_id, canonical_family, source_families, representative, coverage, tuple(sorted(links, key=self._link_key)), EventAnchors.from_article(representative.candidate.article, event_families=source_families), scores, tuple(audits), tuple(collapses), materiality)

    def _exact_identity_groups(self, judged: Iterable[JudgedRouteBCandidate]) -> list[tuple[list[JudgedRouteBCandidate], tuple[PairDecision, ...], tuple[ExactIdentityCollapse, ...]]]:
        ...
        output: list[tuple[list[JudgedRouteBCandidate], tuple[PairDecision, ...], tuple[ExactIdentityCollapse, ...]]] = []
        for group in grouped:
            ...
            collapses = ()
            if len(group) > 1:
                collapses = (ExactIdentityCollapse(
                    canonical_article_id=article_id(representative_article),
                    collapsed_link_ids=tuple(self._impact_link_id(link) for link in sorted(group, key=self._link_key)),
                    companies=tuple(sorted({link.candidate.company for link in group}, key=str.casefold)),
                    exposure_ids=tuple(sorted({link.candidate.exposure_id for link in group})),
                    source_event_families=tuple(sorted({link.decision.event_family for link in group})),
                ),)
            output.append((group, tuple(audits), collapses))
        return output

    @staticmethod
    def _impact_link_id(link: JudgedRouteBCandidate) -> str:
        raw = "|".join((article_id(link.candidate.article), link.candidate.company, link.candidate.exposure_id, link.decision.event_family, link.decision.materiality))
